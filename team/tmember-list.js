...
(() => {
	window.addEventListener("DOMContentLoaded", () => {
		...
		if (pid != null) {
			...
			document
				.querySelector("form[name='team-members']")
				.querySelector("input[name='pid']").value = pid;
...
(() => {
	...
	btnRemove.addEventListener("click", async (e) => {
		...
		const checkedMember = document.querySelector(
...
async function getTeamMeberList(pid) {
	...
	for (let item of result) {
		let createdEle = createRow(item.mid, item.mname, item.username, item.email);
...
function teamMembersClickEvent() {
	...
	ulBody.addEventListener("click", (event) => {
		...
		alert(
			"ul li click : clickedElement.type=>" +
				clickedElement.type +
				", mid" +
				mid
		);
